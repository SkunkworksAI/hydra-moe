...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def update_alphas(self, alphas_dict):
        self.base_model._update_alphas(alphas_dict)
    ...
    def get_prompt_embedding_to_save(self, adapter_name: str):
        ...
        prompt_tokens = (
            self.prompt_tokens[adapter_name].unsqueeze(0).expand(
                1, -1).to(prompt_encoder.embedding.weight.device)
        )
    ...
    def load_adapter(self, model_id: str, adapter_name: str, is_trainable: bool = False, **kwargs: Any):
        ...
        if use_safetensors:
            ...
            adapters_weights = torch.load(
                filename, map_location=torch.device(
    ...
    def create_or_update_model_card(self, output_dir: str):
        ...
        if training_procedure_heading in lines:
            lines.insert(lines.index(training_procedure_heading) +
                         2, training_config_text)
        else:
            lines.append(
        ...
        if framework_block_heading in lines:
            lines.insert(lines.index(framework_block_heading) +
                         2, f"- PEFT {__version__}\n")
        else:
            lines.append(
    ...
    **Attributes**:
    ...
    def _prefix_tuning_forward(
        ...
        **kwargs,
        ...
        fwd_params = list(inspect.signature(
            self.base_model.forward).parameters.keys())
        kwargs.update(
            {
                "input_ids": input_ids,
                "attention_mask": attention_mask,
        ...
        if "past_key_values" in fwd_params:
            ...
            fwd_params = list(inspect.signature(
                transformer_backbone_name.forward).parameters.keys())
            ...
            logits = self.base_model.get_submodule(
                self.cls_layer_name)(pooled_output)
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if labels is not None:
                prefix_labels = torch.full(
                    (batch_size, peft_config.num_virtual_tokens), -100).to(labels.device)
            ...
            if model_kwargs.get("attention_mask", None) is not None:
                ...
                model_kwargs["attention_mask"] = torch.cat(
                    (prefix_attention_mask,
                     model_kwargs["attention_mask"]), dim=1
                )
    ...
    def _prefix_tuning_forward(
        ...
        **kwargs,
        ...
        if "past_key_values" in fwd_params:
            ...
            logits = self.base_model.get_submodule(
                self.cls_layer_name)(sequence_output)
